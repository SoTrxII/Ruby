...
function playsound(filename, volume) {
  dispatcher = ruby.voiceConnections.get(serverId).playFile('./sounds/'
      + filename, {volume: volume}, (error, streamIntent) => {
    if (error) {
      console.error(error);
    }
  });
...
ruby.on("message", function(message) {
  ...
  if (message.content.startsWith('$')) {
    ...
    switch (command) {
      ...
      case 'dramatic':
        ...
        return playsound("inception.mp3");
      ...
      case 'corbeau':
        ...
        return playsound("corbeau.mp3", 0.2);
  ...
  if (mentioned) {
    return message.reply(mentionReply(message.author))
        .then(msg => log(info("ruby has been mentioned by "
...
// } }, { 'trigger': ['YouTube audio', 'musique'], 'reaction': function (data) {
// onYoutubeAudio(data); } }, { 'trigger': 'fin du flux', 'reaction': function (data) {
// dispatcher.end(); } }, { 'trigger': ['volume', 'son'], 'reaction': function (data) {
// onVolumeChange(data); } } ];
