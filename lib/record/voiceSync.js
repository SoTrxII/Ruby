...
/**
Extract from the record log the userId
@param name username as written in the log
**/
...
/**
Extract from the record log the timestamp
@param name username as written in the log
**/
...
/**
Parse timestamps into human readable mesure
@param timeString actual timeStamp
**/
...
/**
One FFMPEG to join them all.
**/
let reassemble = (config) => {
	return new Promise((resolve, reject) => {
		let outputPath = path.join(config.id + ".final");
...
/**
 Sync each user voices in the audiomix.
**/
let assembleUsers = (inputDirectory, podcastTimestamp) => {
	return new Promise ( (resolve, reject) => {
		fs.readdir(inputDirectory, async (err, files) => {
			...
			if(!users.length || ++nbUsersDone >= users.length){
				resolve();
			}
...
exports.sync = async (absoluteDirPath) => {
	console.log("Debut Sync");
  let podcastName = absoluteDirPath.split(path.sep);
  podcastName = podcastName[podcastName.length - 1];
  let podcastTimestamp = extractTimestamp(podcastName);
  await assembleUsers(absoluteDirPath, podcastTimestamp);
	console.log("Fin Sync");
}
//exports.sync('/tmp/226641484827459586-1504178479743');
